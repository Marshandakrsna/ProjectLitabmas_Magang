    ...
    public class UserDAO
    {
        ...
        public string getRoleByUsername(string uname)
        {
            try
            {
                ...
                string query = "select [DESKRIPSI] from [SIATMAX].[siatmax].[REF_ROLE] r join [SIATMAX].[siatmax].[TBL_USER_ROLE] s on r.id_role=s.id_role where s.npp = '" + uname + "'and s.ID_SISTEM_INFORMASI=4 ";
                if (sqlConn.State.ToString() != "Open")
                    sqlConn.Open();
                cmd = new SqlCommand(query, sqlConn);
                reader = cmd.ExecuteReader();
        ...
        public string getEmailProdiByNPPDosen(string NPP)
        {
            try
            {
                string query = "select kk.EMAIL from simka.MST_KARYAWAN k join siatmax.MST_UNIT u on k.ID_UNIT_AKADEMIK=u.ID_UNIT join simka.MST_KARYAWAN kk on kk.NPP=u.NPP where k.NPP= '" + NPP + "' ";
        ...
        public string getEmailDekanByNPPProdi(string NPP)
        {
            try
            {
                string query = "select KK.EMAIL from simka.MST_KARYAWAN k join siatmax.MST_UNIT u on k.ID_UNIT=u.ID_UNIT join simka.MST_KARYAWAN kk on kk.NPP=u.NPP where k.NPP= '" + NPP + "' ";
        ...
        public string getEmailStaff()
        {
            try
            {
                string query = "select m.EMAIL from simka.MST_KARYAWAN m join siatmax.TBL_USER_ROLE u on u.NPP=m.NPP where u.ID_ROLE=1 and u.ID_SISTEM_INFORMASI=4";
        ...
        public string getEmailKALPPM()
        {
            try
            {
                string query = "select m.EMAIL from simka.MST_KARYAWAN m join siatmax.TBL_USER_ROLE u on u.NPP=m.NPP where u.ID_ROLE=17 and u.ID_SISTEM_INFORMASI=4";
        ...
        public string getEmailPustakawan()
        {
            try
            {
                string query = "select m.EMAIL from simka.MST_KARYAWAN m join siatmax.TBL_USER_ROLE u on u.NPP=m.NPP where u.ID_ROLE=5 and u.ID_SISTEM_INFORMASI=4";
        ...
        public DataSet getAllDatauser()
        {
            try
            {
                string query = "select k.NPP,k.NAMA,r.DESKRIPSI from simka.MST_KARYAWAN k join siatmax.TBL_USER_ROLE u on u.NPP=k.NPP join siatmax.REF_ROLE r on r.ID_ROLE=u.ID_ROLE where  r.DESKRIPSI= 'Dosen' and u.ID_SISTEM_INFORMASI=4  or r.DESKRIPSI='Assesor' and u.ID_SISTEM_INFORMASI=4 order by r.DESKRIPSI";
                if (sqlConn.State.ToString() != "Open")
                    sqlConn.Open();
        ...
        public DataSet getUserRoleAssessor()
        {
            try
            {
                ...
                string query = "select * from simka.MST_KARYAWAN m join  [SIATMAX].[siatmax].[TBL_USER_ROLE] s on s.NPP=m.NPP join [SIATMAX].[siatmax].[REF_ROLE] r  on r.id_role=s.id_role where r.DESKRIPSI='Assesor' and s.ID_SISTEM_INFORMASI=4";
                if (sqlConn.State.ToString() != "Open")
                    sqlConn.Open();
                SqlDataAdapter adapter = new SqlDataAdapter(query, sqlConn);
                adapter.Fill(dataset);
        ...
        public DataTable getAllMenu(string role)
        {
            ...
            SqlCommand cmd = new SqlCommand(@"
                    ...
                    WHERE (siatmax.REF_ROLE.DESKRIPSI = @role) and TBL_SI_SUBMENU.IsActive=1 and TBL_SI_MENU.ID_SISTEM_INFORMASI=4
                     union all                                      SELECT  distinct TBL_SI_SUBMENU.ID_SI_SUBMENU, TBL_SI_SUBMENU.DESKRIPSI                           ,TBL_SI_SUBMENU.LINK
                      ...
                      siatmax.TBL_SI_SUBMENU ON siatmax.TBL_ROLE_SUBMENU.ID_SI_SUBMENU = siatmax.TBL_SI_SUBMENU.ID_SI_SUBMENU INNER JOIN
                      siatmax.TBL_SI_MENU ON siatmax.TBL_SI_SUBMENU.ID_SI_MENU = siatmax.TBL_SI_MENU.ID_SI_MENU
                    WHERE (siatmax.REF_ROLE.DESKRIPSI = @role) and TBL_SI_SUBMENU.IsActive=1 and TBL_SI_MENU.ID_SISTEM_INFORMASI=4 ) menu", sqlConn);
        ...
        public DataSet getMenu(string role)
        {
            ...
            SqlCommand cmd = new SqlCommand(@"SELECT DISTINCT 
                    ...
                    FROM siatmax.TBL_ROLE_SUBMENU INNER JOIN
                      ...
                      siatmax.TBL_SI_SUBMENU ON siatmax.TBL_ROLE_SUBMENU.ID_SI_SUBMENU = siatmax.TBL_SI_SUBMENU.ID_SI_SUBMENU INNER JOIN
                      siatmax.TBL_SI_MENU ON siatmax.TBL_SI_SUBMENU.ID_SI_MENU = siatmax.TBL_SI_MENU.ID_SI_MENU
                    WHERE (siatmax.REF_ROLE.DESKRIPSI = @role) and TBL_SI_SUBMENU.IsActive=1 and TBL_SI_MENU.ID_SISTEM_INFORMASI=4", sqlConn);
