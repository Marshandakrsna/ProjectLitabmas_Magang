    ...
    public class UserDAO
    {
        ...
        public string getRoleByUsername(string uname)
        {
            try
            {
                ...
                string query = "select [DESKRIPSI] from [SIATMAX].[siatmax].[REF_ROLE] r join [SIATMAX].[siatmax].[TBL_USER_ROLE] s on r.id_role=s.id_role where s.npp = '" + uname + "'and s.ID_SISTEM_INFORMASI=8 ";
                if (sqlConn.State.ToString() != "Open")
                    sqlConn.Open();
                cmd = new SqlCommand(query, sqlConn);
                reader = cmd.ExecuteReader();
        ...
        public DataSet getUserRoleAssessor()
        {
            try
            {
                ...
                string query = "select * from simka.MST_KARYAWAN m join  [SIATMAX].[siatmax].[TBL_USER_ROLE] s on s.NPP=m.NPP join [SIATMAX].[siatmax].[REF_ROLE] r  on r.id_role=s.id_role where r.DESKRIPSI='Assesor' and s.ID_SISTEM_INFORMASI=8";
                if (sqlConn.State.ToString() != "Open")
                    sqlConn.Open();
                SqlDataAdapter adapter = new SqlDataAdapter(query, sqlConn);
                adapter.Fill(dataset);
